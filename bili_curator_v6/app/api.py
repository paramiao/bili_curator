...
async def get_subscription_expected_total(subscription_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # 公共参数：UA/Referer/重试/轻睡眠（与下载链路保持一致）
        common_args = [
            'yt-dlp',
        ...
        # 优先：手动分页统计，降低一次性请求规模（每段100，段间延时2-4s）
        try:
            chunk_size = 100
            max_chunks = 50  # 安全上限
            total_count = 0
        ...
        # 尝试B：全量JSON（-J），有时包含 entries 或 playlist_count
        if expected_total is None:
            try:
                ...
                if rc == 0:
                    try:
                        data = json_lib.loads(out.decode('utf-8', errors='ignore') or '{}')
                        if isinstance(data, dict):
                            # 优先使用 n_entries / entries
                            expected_total = data.get('n_entries')
                            if expected_total is None:
                                entries = data.get('entries')
                                if isinstance(entries, list):
                                    expected_total = len(entries)
                            # 回退 playlist_count（部分B站提取器会给）
                            if expected_total is None:
        ...
        # 尝试D：--dump-json --flat-playlist 行式解析（与V4一致）
        if expected_total is None:
            try:
        ...
        if expected_total is None:
            detail_msg = f"无法解析合集总数; last_error={last_err!s}"
        ...
        return {"expected_total_videos": int(expected_total)}
...
async def parse_collection_info(request: dict, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if pl_proc.returncode == 0:
            ...
            if name:
                try:
                    ...
                    pass
                return {"name": name, "uploader": uploader, "playlist_title": playlist_title}
        ...
        fe_stdout, fe_stderr = await fe_proc.communicate()
        if fe_proc.returncode != 0:
            error_msg = fe_stderr.decode('utf-8', errors='ignore')
        ...
        for line in fe_stdout.decode('utf-8', errors='ignore').strip().split('\n'):
            ...
            try:
                ...
                playlist_title = (video_info.get('playlist_title') or '').strip()
