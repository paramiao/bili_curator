...
async def get_subscription_expected_total(subscription_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if process.returncode != 0:
            ...
            raise HTTPException(status_code=502, detail=f"yt-dlp失败: {err}")

        data = json_lib.loads(stdout.decode('utf-8', errors='ignore'))
        # yt-dlp 可能提供 n_entries；否则用 entries 长度兜底
        expected_total = None
        if isinstance(data, dict):
        ...
        if expected_total is None:
            raise HTTPException(status_code=502, detail="无法解析合集总数")
