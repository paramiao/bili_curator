...
### 当前状态（2025-08-27）
- **功能侧**：V7 STRM 全链路完成，后端95%+、API 100%、前端90% 已可用
- **工程侧**：依赖版本收敛与Docker部署验证进行中；端口统一为 8080
- **已知事项**：
  - 依赖补齐与封顶：aiohttp/psutil/pydantic-settings 等版本范围统一
  - 调度一致性：为所有路径补充 STRM 分流检查，增量空ID回退日志与测试
  - STRM元数据：修复 `_get_uploader_videos` 目录命名依赖的元数据获取
  - 容量统计：API 三级回退（DB字段→磁盘文件），并安排 refresh-sizes 任务

### 下一步计划（P0→P1）
- **P0 稳定收尾（本周）**
  - 依赖修复与上限：补齐 aiohttp/psutil/pydantic-settings 并设置上限；校验 yt-dlp 策略
  - STRM 元数据：修正 `_get_uploader_videos`，保证 title/uploader 完整与目录命名稳定
  - 容量统计：API 三级回退（DB→磁盘）+ 注册 `refresh-sizes` 定时任务
  - Metrics：确保 `METRICS_REFRESH_INTERVAL_MINUTES` 环境变量优先生效（ENV > DB > 默认）
  - 部署验证：Docker 端到端自检（FFmpeg、网络、健康检查、STRM 路径权限）
- **P1 发布前增强（1-2周）**
  - 调度器全路径 STRM 分流一致性复核（含未启用增量路径）；补日志
  - 订阅8 增量回退用例测试：`remote_ids=[]` 时强制回退传统路径
  - 文档与 compose 统一：端口统一 8080、移除历史 `STRM_PROXY_PORT`、完善 `.env.example`
